...
const { EARLY_SLOTS, FADE_S } = await import(process.argv[4]);
let s = 5;
...
function run(switchAt) {
  const options = { slots: EARLY_SLOTS, fade: FADE_S, sampleRate: rate };
  const convolver = createPartitionedConvolver(options);
  const gains = createRing(EARLY_SLOTS, FADE_S);
...
// each gain ramped across a block from its value at the block's two ends.
const switched = run(80);
const gain = (slot, t) => {
  const block = Math.floor(t / BLOCK);
  const [from, to] = [block, block + 1].map((b) => switched.gains.gainAt(slot, (b * BLOCK) / rate));
  return from + ((to - from) * (t - block * BLOCK)) / BLOCK;
};
worst = 0; scale = 0;
for (let ear = 0; ear < 2; ear++) {
  for (let t = Math.ceil(0.13 * rate); t < n; t++) {
    const want = gain(0, t) * yA[ear][t] + gain(1, t) * yB[ear][t];
