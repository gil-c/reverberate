"""The engine on any devices gives the numpy code's answer.
...
from reverberate.compute import Devices, cuda_available
...
def test_on_the_host_any_number_of_cores_gives_the_one_process_answer(cores: int) -> None:
    ...
    found = paths_on_devices(scene, tree, RECEIVERS, settings, devices=devices)
    grid = occluder_grid(scene)
    for receiver, paths in zip(RECEIVERS, found, strict=True):
        alone = paths_for(scene, tree, receiver, settings, grid=grid)
        np.testing.assert_array_equal(paths.image, alone.image)
        np.testing.assert_allclose(paths.points, alone.points)
    rays = RaySettings(rays=200, duration_s=0.1, seed=2)
    histogram = histogram_on_devices(scene, SOURCE, RECEIVERS, rays, devices=devices)
    alone_histogram = trace(scene, SOURCE, RECEIVERS, rays)
    np.testing.assert_array_equal(histogram.hits, alone_histogram.hits)
    np.testing.assert_array_equal(histogram.energy, alone_histogram.energy)
    np.testing.assert_array_equal(histogram.moments, alone_histogram.moments)
...
def test_one_card_and_every_card_give_the_same_field() -> None:
    ...
    h1 = histogram_on_devices(scene, SOURCE, RECEIVERS, rays, devices=Devices((0,)))
