...
from reverberate.mirror.diffract import DiffractionSettings, diffracted_paths
from reverberate.mirror.occupancy import maekawa_db
...
def test_a_corner_snaps_onto_the_edge_it_stands_near() -> None:
    ...
    from reverberate.mirror.edges import diffracting_edges, snap_to_edges
    ...
    moved, on_edge = snap_to_edges([c.copy() for c in corners], edges, 0.5)
    assert len(moved) == 3
    ...
    far, none = snap_to_edges([c.copy() for c in corners], edges, 1e-6)
    assert not any(none)
