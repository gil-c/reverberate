...
def test_the_join_keeps_each_solver_in_its_own_band() -> None:
    """Under the cutoff the low side comes back untouched, over it the levelled high side."""
    rate = 48000.0
    rng = np.random.default_rng(0)
    samples = 24000
    low = rng.standard_normal((4, samples))
    high = rng.standard_normal((4, samples))
    joined, record = blend(low, high, rate, Crossover(coherent_s=0.0))
    gain = 10.0 ** (record["applied_gain_db"] / 20.0)
    ...
    np.testing.assert_allclose(spectra["j"][:, over], gain * spectra["h"][:, over], rtol=1e-3)
...
def test_the_seam_is_measured_and_levelled() -> None:
    ...
    joined, record = blend(low, high, rate, crossover)
...
def test_a_shared_arrival_is_rebuilt_and_a_shared_tail_is_not() -> None:
    ...
    joined, record = blend(low, low, rate, Crossover())
