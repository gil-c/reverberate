...
The decoder here is the library's own, designed for a synthetic measured head
and exported as the page reads it, so it is centred as the real one is.
...
def test_the_early_and_late_parts_sum_to_the_whole_response(tmp_path: Path) -> None:
    ...
    # The measured head's decoder, from a synthetic head file: the only one
    # the exporter writes once the sphere head is gone.
    head = _Head().a_file(tmp_path)
    export_decoders(tmp_path / "decoders", measured_path=head, order=3, filter_length=512)
    ...
    out = subprocess.run(
        [
            ...
            str(AUDIO / "decode.js"),
            str(AUDIO / "sh.js"),
            str(tmp_path / "decoders" / "measured"),
        ],
        capture_output=True,
        text=True,
        check=False,
