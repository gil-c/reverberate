 ...
 */
import { createRing, curveValue } from "../../src/reverberate/viz/app/audio/ring.js";
...
export function createPair({ sampleRate, fade, count, hot = false }) {
  ...
  function gainAt(slot, t) {
    ...
    return curve ? curveValue(curve, t) : 0;
  }
  ...
  return {
    ...
    swap(at, response, about = null) {
      ...
      // Copies: the ring's curves are its own storage, good until its next call.
      for (const { slot: index, start, duration, samples } of played) {
        curves[index].push({ start, duration, samples: Float32Array.from(samples) });
      }
    },
