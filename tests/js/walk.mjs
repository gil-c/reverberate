 ...
 * What is modelled rather than shared is the schedule around those rules:
 * a pose arrives at every animation frame the listener moves in, and none
 * while they are still; a request is issued at most every `minIntervalS` and
 * never while the worker is busy; the worker is busy for as long as a decode
 * costs, and the response takes over then; the late part is decoded again
 * once the head has been still for `stillMs`, as the page's timer does.
 */
...
export async function renderWalk({ field, decoderFilters, poses, sampleRate, variant = {} }) {
  ...
  const moved = (a, b) => !a || ["x", "y", "z", "yaw", "pitch"].some((key) => Math.abs(a[key] - b[key]) > 1e-9);
  ...
  for (const pose of poses) {
    const now = pose.t;
    if (!moved(lastPose, pose)) {
      // No pose reaches the page while the listener is still; its timer
      // decodes the late part again for the head once they have been still
      // for `stillMs`.
      if (!stillDone && now - lastMoved >= settings.stillMs / 1000) {
        stillDone = true;
        const head = headMatrix(pose.yaw, pose.pitch);
        if (lateKey && !sameHead(head, lateHead)) renderLate(now, lateKey, head);
      }
      continue;
    }
    lastPose = pose;
    ...
    const position = field.cellAt(pose.x, pose.y, pose.z, cell === null ? REACH_M : HOLD_M);
    ...
    // The late part, by the distance walked. A move that is due but too soon
    // after the last is taken at the next request; the page's own timer
    // takes it even if none comes, which a walk that keeps moving never needs.
    const moves = settings.tailEveryCell
      ? position !== cell
      : tailWait(tailState, pose, position, field.index.rooms, now) === 0;
    cell = position;
    if (moves) {
