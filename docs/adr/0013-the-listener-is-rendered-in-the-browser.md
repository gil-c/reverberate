...
blocks and transformed, so the audio thread only multiplies and adds. The
late part stays on the browser's `ConvolverNode`s: it is replaced every two
metres at most, and it is a second of diffuse reverberation.

What that costs the audio thread was measured in the browser, with real
responses arriving every 40 ms for every source, as while walking: a block
of every source's early part takes 4 per cent of the time a block lasts for
one source, 23 for five (30 at the ninety-ninth percentile), 49 for ten (64).
Five sources turning at full speed, every one on a tone, showed no transient
at all. The thread allocates nothing once it is going: a filter is copied
into storage of its own slot, and the buffers it came in are sent back to
the thread that made them, so no collection runs where a pause of three
milliseconds is a click; the ring reuses its curves the same way.

An `AudioWorklet` exists only in a secure context: https, or the loopback
address, which is how `serve_room.py` serves the page. Opened over plain http
from another machine, the page has no early part, and says so rather than
play the tail alone.
...
Anchored to the pose, it strays by under one. Each ramp starts from the value
the node reports: tracking the ramp in the page instead, which would spare a
browser that reports it late, measured worse in Chrome -- 100 transients in
25 s of walking through a tone against 36. Firefox and Safari are untested.
...
over a quarter of a second. A move that falls due within the 0.7 s is taken
when they are up, on a timer: a listener who steps through a door and stops
sends no pose to take it. Each swap of it clears a `ConvolverNode` and
...
1.1 m to begin with** -- the nearest by distance, not the first found: the
search goes out a shell of the lattice at a time, and a corner of one shell
can be further than a face of the next. The solver writes no cell inside the furniture or
...
| --- | --- | --- | --- | --- | --- | --- |
| the page before, near enough | 10.6 | 6.5 | 4.2 | 7.2 | 12.2 | 78 |
...
the page is at it: 0.01, 0.14 and 0.01 on level, timbre and interaural
level, against 0, 0.22 and 0.01, where on `ConvolverNode`s it measured 0.64,
1.06 and 0.73. A fade of 80 ms instead of 120 would put the sound a degree
...
convolution, `tests/test_worklet_js.py` its two shells against a stand-in Web
Audio, and `tests/test_audio_smoothness_js.py` the ring, the rules of
