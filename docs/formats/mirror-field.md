...
| `mirror/scene.npz`, `scene.json` | The derived geometry: reflector facets and triangles, occluder triangles, labels, materials, rules, census, key. |
| `mirror/paths_<S>.npz` | Every point's validated paths, concatenated: `offsets` (points + 1), and per path `image`, `order`, `length_m`, `direction` (3), `gain` (bands), `points` (vertices, 3), `sequence` (facets). |
