...
# REVERBERATE PATCH 7 -- np.float was removed in numpy 1.20 and upstream still
# reads it here, so importing this module raises AttributeError on every numpy
# this project can install.  The fix was already in the checkout, applied by
# hand and recorded nowhere: `git status` in data/vendor/pffdtd showed
# common/myfuncs.py modified, outside PATCHED_FILES, so ensure_patched neither
# knew about it nor would restore it.  A fresh clone at the pinned commit could
# not voxelise at all, and nothing in the repository said why.  Whole file,
# pinned, like every other replacement.
