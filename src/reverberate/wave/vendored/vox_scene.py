...
class VoxScene:
    ...
    def calc_adj(self,Nprocs=None):
        ...
        back_side = ((dv>0) & (rg.mat_side[tidx_bn]==1)) | ((dv<0) & (rg.mat_side[tidx_bn]==2))
        mat_bn[back_side] = -1
        # REVERBERATE PATCH 5 -- seal the non-air side, do not merely silence it.
        #
        # Upstream takes the material away from these nodes and leaves them
        # adjacent to their neighbours, which makes the inside of every closed
        # object a perfectly rigid air cavity: absorption zero, Q enormous, and
        # simulated in full, because KernelAirCart reads all six neighbours
        # unconditionally. Measured on one bedroom that is 141 cavities holding
        # 4.6 m3, the largest resonating at 133 Hz, and it put 3.64 s of decay
        # into the 125 Hz band against 0.25 s at mid frequencies.
        #
        # There is no air behind a solid surface, so the node has nothing to be
        # adjacent to. With adj_bn all false the engine computes K_bn = 0 and
        # the update degenerates to u0 = 2*u1 - u0: a node that starts at zero
        # stays there, and -- the point -- it stops reading its neighbours, so
        # nothing behind the surface can be driven either.
        adj_bn[back_side,:] = False
