/** The mirror's audit view: the derived scene the geometric engine read.
 *
 * Its own view, not an overlay: the reflecting facets and the decimated
 * occluders, each drawn as the scene's own triangles (the server checked the
 * files against `mirror/scene.npz` triangle for triangle and label for label,
 * and refuses them otherwise), the receivers the engine was given, and, at
 * the listener's cell, the paths the image source model validated.
 *
 * The facets are coloured by material label with the grid's palette, so a
 * label has one colour in both audits, or by facet kind (shell floor, wall,
 * ceiling, furniture). A reflector triangle's facet is found from the
 * facets' order and triangle counts, which the server also checked.
 */
...
export function createMirrorLayers(THREE, { onLoaded } = {}) {
  ...
  let check = null;
  let quadMeta = null;
  let facetEnds = null; // cumulative triangle counts, one per facet
  ...
  function kindColours(mesh) {
    ...
    let quad = 0;
    for (const facet of check.facets) {
  ...
  return {
    ...
    /** The mirror record of run.json; resolves once the layers' index is read. */
    async load(run) {
      this.clear();
      if (!run || !run.mirror) return null;
      check = run.mirror.check || null;
      if (check && check.facets) {
        facetEnds = [];
        let total = 0;
        for (const facet of check.facets) facetEnds.push((total += facet.triangles));
      }
      ...
      // The server links the layers only when they are the scene's.
      if (run.mirror.audit && check && !check.problems.length) {
        const mine = `${run.baseUrl}/${run.mirror.audit}`;
        base = mine;
        const index = await fetch(`${mine}/layers.json`).then((r) => (r.ok ? r.json() : null));
        if (base !== mine) return check;
        if (index && index.key === check.key) quadMeta = index.layers;
      }
      return check;
    },
    ...
    /** The visible meshes a click may pick. */
    pickable: () => ["reflectors", "occluders"].map((n) => layers[n]).filter((m) => m && m.visible),
    /** The facet of reflector triangle `quad`. */
    facetOf(quad) {
      if (!facetEnds || !facetEnds.length) return null;
      let lo = 0;
      let hi = facetEnds.length - 1;
      while (lo < hi) {
        const mid = (lo + hi) >> 1;
        if (facetEnds[mid] > quad) hi = mid;
        else lo = mid + 1;
      }
      return facetEnds[lo] > quad ? lo : null;
    },
    isReflectors: (mesh) => mesh === layers.reflectors,
