...
const TELEPORT_S = 0.6 / SOUND_SPEED_M_S;
...
export function createEngine({ onError = (message) => console.error(message) } = {}) {
  ...
  function release(target, slot) {
    ...
    slot.timer = setTimeout(() => {
      const live = target.order.live;
      if (live === null || target.slots[live] !== slot) slot.convolver.buffer = null;
    }, target.fade * 1000 + 50);
  }
  ...
  return {
    ...
    setPropagation(id, seconds, poseAt) {
      ...
      // Read back from the node. Tracking the ramp here instead, which would
      // spare a browser that reports a ramp's value late, measured worse in
      // Chrome: 100 transients in 25 s of walking through a tone, against 36.
    ...
    setTail(id, response, startSamples) {
      const source = entry(id);
      // The same at every swap but after a change of the split in the
      // settings, which restarts every response anyway.
      if (source.tailDelay !== startSamples) {
        source.delay.delayTime.value = startSamples / this.sampleRate;
        source.tailDelay = startSamples;
      }
