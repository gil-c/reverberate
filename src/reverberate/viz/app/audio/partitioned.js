 ...
 * slot is an instant change of filter, and that clicks too. A slot's gain is
 * read off the ring at the two ends of each block and ramped linearly between
 * them: the ring's curves are sampled every two milliseconds, a block lasts
 * under three, and the ramp misses the curve by a few ten-thousandths.
 *
 * It runs on the audio thread, so it allocates nothing once it is going: a
 * filter is copied into its slot's own storage, and the storage it came in is
 * the caller's to give back to the thread it came from. A collection pause
 * longer than a block, three milliseconds, is a click of its own.
 *
...
export function createPartitionedConvolver({ slots, fade, sampleRate }) {
  const ring = createRing(slots, fade);
  // Each slot's filter, in storage of its own, grown only when a longer
  // filter arrives -- which is a change of the page's settings, not a walk.
  const filters = Array.from({ length: slots }, () => ({ partitions: 0, re: null, im: null }));
  ...
  return {
    /** A new filter, from `partitionFilter`, taking over from `time`; it is
     *  copied, and the caller keeps `filter`. */
    setFilter(filter, time) {
      capacity(filter.partitions);
      const into = filters[ring.take(time).slot];
      if (!into.re || into.re.length < filter.re.length) {
        into.re = new Float32Array(filter.re.length);
        into.im = new Float32Array(filter.im.length);
      }
      into.re.set(filter.re);
      into.im.set(filter.im);
      into.partitions = filter.partitions;
    },
    ...
    process(input, left, right, time) {
      ...
      left.fill(0);
      right.fill(0);
      const end = time + BLOCK / sampleRate;
      for (let slot = 0; slot < slots; slot++) {
        const filter = filters[slot];
        if (!filter.partitions) continue;
        const from = ring.gainAt(slot, time);
        const to = ring.gainAt(slot, end);
        ...
        for (let n = 0; n < BLOCK; n++) {
          const gain = from + ((to - from) * n) / BLOCK;
