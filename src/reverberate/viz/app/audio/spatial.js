...
/** Whether the late part is to be fetched again, from `position`, for a
 *  listener at `pose` at `now` seconds; `tail` is where and when it was last
 *  fetched, `{ position, at, fetchedAt }`, and `rooms` the field's own. */
export function tailMoves(tail, pose, position, rooms, now) {
  if (tail.position === null) return true;
  ...
  return far && now - tail.fetchedAt >= TAIL_INTERVAL_S;
}
...
export function createSpatial({ engine, workerUrl, onRendered, onStatus, onCell }) {
  ...
  async function request(id, pose) {
    ...
    try {
      ...
      if (tailMoves(state.tail, pose, position, state.field.index.rooms, now)) {
