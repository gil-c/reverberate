...
:mod:`reverberate.air`, and reports the decay per octave band **and per
...
from reverberate.air import Atmosphere, apply, attenuation_db_per_m
...
def analytic_t60_s(
    ...
    **It is a prediction, not the measurement, and it reads long.** The
    ...
    """
    air_alone = 60.0 / (
        float(np.asarray(attenuation_db_per_m(centre_hz, atmosphere))) * sound_speed_m_s
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"{atmosphere.relative_humidity_pct:g} per cent relative humidity"
    )
