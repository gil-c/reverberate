...
from reverberate.audio import apply_air_absorption as apply_air
...
def build(
    ...
    *,
    ...
    mid_wet = np.atleast_2d(
        apply_air(
            mid_response.ir, float(sample_rate), atmosphere=atmosphere, sound_speed_m_s=sound_speed
        )
    )
    high_wet = np.atleast_2d(
        apply_air(
            high_response.ir, float(sample_rate), atmosphere=atmosphere, sound_speed_m_s=sound_speed
        )
    )
    ...
    gain = audio.peak_gain(*[block for group in wet_audio for block in group])
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = build(
        ...
        args.high,
        args.out,
        atmosphere=Atmosphere(humidity_percent=args.humidity_percent),
    )
